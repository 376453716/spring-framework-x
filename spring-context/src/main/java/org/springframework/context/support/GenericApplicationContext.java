...
/**
 * 通用应用上下文,实现了bean注册接口,继承{@link AbstractApplicationContext}
 * 拥有DefaultListableBeanFactory实例，不指定bean定义的形式(如可通过XMl或注解定义)。
 * 实现了bean定义注册器接口，为了bean定义读取器对其进行bean的注册等操作
 *
 * <p>典型的用法是通过BeanDefinitionRegistry接口注册各种Bean定义，
 * 然后调用refresh()以使用应用程序上下文语义初始化这些Bean
 ...
 * refresh()只能被调用一次。
 *
 * <p>对于XML Bean定义的典型情况，只需使用{@link ClassPathXmlApplicationContext}或{@link FileSystemXmlApplicationContext}，
 * 它们比较容易设置-但灵活性较差，因为您只能将标准资源位置用于XML Bean`定义，而不是混合使用任意的bean定义格式。
 * 在web环境中，等效项是 {@link org.springframework.web.context.support.XmlWebApplicationContext}。
 *
 * <p>对于应该以可刷新方式读取特殊bean定义格式的自定义应用程序上下文实现，
...
public class GenericApplicationContext extends AbstractApplicationContext implements BeanDefinitionRegistry {
	...
	/**
	 * 是否自定义类加载器，设置自定义类加载器后，读取资源会根据自定义类加载器信息获取
	 */
