...
/**
 * 一个简单的类型过滤器{@link TypeFilter}，它与具有给定注释的类匹配，还会检查继承的注释。
 * <p/>
 * 默认情况下，匹配逻辑将镜像{@link AnnotationUtils#getAnnotation(java.lang.reflect.AnnotatedElement, Class)},
 * 对于单一级别的的annotation。支持present或meta-present ？
 * 对注解对搜索可能会被禁用。
 * 同样，可以选择启用对接口的注释的搜索。
